...
class NumeroALetras{
     ...
     * @param int|float $number
     * @param int       $decimals
     * @param string    $currency aquí habría que mandar PESOS como parámetro
     *
     * @return string
     */
    public function toInvoice($number, $decimals = 2, $currency = ''){
        ...
        if (!empty($splitNumber[1])) {
            $splitNumber[1] .= '/100 M.N.';
        } else {
            $splitNumber[1] = '00/100 M.N.';
        }
    ...
    private function convertNumber($number){
        ...
        if (($number < 0) || ($number > 999999999)) { // 999,999,999
