...
//
	...
	if ( isset($datos["aCampos"])){
		...
		$aColumns = explode(",", $cCampos); // $aCampos; //
	}else{
		$aColumns = explode(",", $cCampos);
		$aCampos = $aColumns;
	}
	...
	*/
	...
	if ($campo != null && $campo!="") {
		$where = construye_where($campo, $aCampos,$aTipos);
		if ($join!=""){
			if ($where==""){
				$where .= " where ( $join ) ";
			}else{
